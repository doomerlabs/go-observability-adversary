#!/usr/bin/env node
import { createRequire as __goObservabilityCreateRequire } from 'node:module'; const require = __goObservabilityCreateRequire(import.meta.url);
...
async function listInScopePaths(repoPath, change, options = {}) {
  ...
  if (change !== null && change.scanMode === "changed") {
    candidates4 = change.changedFiles.map(normalizePath3);
  } else {
...
var domain = {
  ...
  rules: [
    ...
    },
    {
      ...
      severity: "high",
      confidence: "high",
      summary: (count) => `${count} duration metric observation${count === 1 ? "" : "s"} disagree with the declared unit.`,
      whyItMatters: "Metric names are the unit contract for dashboards and histogram buckets; recording another scale silently corrupts every query.",
    ...
    },
    {
      ...
      severity: "high",
      confidence: "high",
      summary: (count) => `${count} metric label site${count === 1 ? "" : "s"} use request- or user-specific values.`,
      whyItMatters: "Every unique label set creates a new time series; user IDs, raw paths, and error strings explode cardinality.",
      impact: "Metrics backends OOM or become unqueryable under real traffic.",
      recommendation: "Use route templates, bounded enums, and status classes as labels; put unbounded data in logs/traces."
    },
    {
      ...
      severity: "high",
      confidence: "high",
      summary: (count) => `${count} span start${count === 1 ? "" : "s"} lack a deferred End in the owning scope.`,
    ...
    },
    {
      ...
      severity: "high",
      confidence: "high",
      summary: (count) => `${count} recover site${count === 1 ? "" : "s"} swallow panics without telemetry.`,
      whyItMatters: "Silent panic swallowing turns crashes into invisible data corruption \u2014 the worst observability defect.",
      impact: "Production failures leave no log, metric, or trace signal.",
      recommendation: "Log the recovered value with a stack, increment a panic counter, and return an error \u2014 never a bare recover."
    },
    {
      ...
      severity: "medium",
      confidence: "high",
      summary: (count) => `${count} slog call${count === 1 ? "" : "s"} have unbalanced or non-string keys.`,
    ...
    },
    {
      ...
      severity: "medium",
      confidence: "high",
      summary: (count) => `${count} span start${count === 1 ? "" : "s"} discard the caller's trace context.`,
      ...
      recommendation: "Thread the request context; use `context.WithoutCancel` for deliberate post-response work."
    },
    {
      ...
      severity: "medium",
      confidence: "high",
      summary: (count) => `${count} metric registration${count === 1 ? "" : "s"} run on a request or loop path.`,
  ...
  noRiskSummary: "The reviewed telemetry uses bounded dimensions, ended spans, and propagated context.",
...
function hasDeferredSpanEnd(source, assignIndex, spanVar) {
...
function recoverSwallowSignals(file) {
  const signals = [];
  const source = file.current;
  const deferRecover = /\bdefer\s+func\s*\([^)]*\)\s*\{([\s\S]{0,400}?)\}(?:\s*\(\s*\))?/g;
  let match;
  while ((match = deferRecover.exec(source)) !== null) {
    const body2 = match[1] ?? "";
    if (!/\brecover\s*\(/.test(body2)) continue;
    const reports = /\b(?:log|slog|logger|fmt)\.\w+\s*\(/.test(body2) || /\.Error\s*\(/.test(body2) || /\bpanic\s*\(/.test(body2) || /\bRecordError\b|\bInc\s*\(/.test(body2) || /\bdebug\.Stack\s*\(/.test(body2);
    if (reports) continue;
    const line = source.slice(0, match.index ?? 0).split("\n").length;
    signals.push({
...
function slogArgsMismatchSignals(file) {
  const signals = [];
  const re = /\b(?:slog\.(?:Info|Warn|Error|Debug|InfoContext|WarnContext|ErrorContext|DebugContext)|(?:\w+)\.(?:Info|Warn|Error|Debug)Context)\s*\(([^;]*)\)/g;
  file.current.split("\n").forEach((line, index) => {
    ...
    const args2 = splitTopLevelArgs(argsSrc);
    if (args2.length === 0) return;
    let start2 = 1;
    if (/Context\s*\(/.test(line)) start2 = 2;
    const kvs = args2.slice(start2);
...
function registerInRequestSignals(file) {
  if (!/\b(?:MustRegister|promauto\.New|prometheus\.New\w*Vec)\b/.test(file.current)) {
    return [];
  }
  const signals = [];
  ...
  for (let i2 = 0; i2 < lines.length; i2 += 1) {
    ...
    if (inFunc !== null && inFunc !== "init" && /\b(?:prometheus\.)?MustRegister\s*\(|\bpromauto\.New\w*\s*\(|\bprometheus\.New\w*Vec\s*\(/.test(
      line
    )) {
      signals.push({
...
function standardAliases(root, source) {
  ...
  for (const spec of descendants(root, "import_spec")) {
    ...
    const match = /^(?:([A-Za-z_]\w*|[_.])\s+)?["`]([^"`]+)["`]$/.exec(text2);
...
function stringValue(node, source) {
  ...
  return text2.length >= 2 ? text2.slice(1, -1).toLowerCase() : text2.toLowerCase();
}
function unboundedValueKind(argument, source) {
  if (isStringLiteral(argument)) return void 0;
  const text2 = normalize(sourceText(argument, source));
  if (/\.URL\.(?:Path|String\(\))$/i.test(text2)) return "request-url";
  if (/\buser_?id\b/i.test(text2)) return "user-id";
  if (/\brequest_?id\b/i.test(text2)) return "request-id";
  if (/\berr(?:or)?\.Error\(\)$/i.test(text2)) return "error-text";
  return void 0;
}
  ...
  for (const fn of functions) {
    ...
    for (const block of blocks) {
      ...
      for (let index = 0; index < statements.length - 1; index += 1) {
        ...
        if (sentinel === void 0 || !sentinels.has(sentinel) || bindingDeclaredBefore(fn, sentinel, returned.startIndex, source)) continue;
...
function staticBoolean(node, source) {
  let text2 = sourceText(node, source).replace(/\s+/g, "");
  while (text2.startsWith("(") && text2.endsWith(")")) text2 = text2.slice(1, -1);
...
function metricContracts(files) {
  ...
  for (const file of files) {
    ...
    for (const match of file.current.matchAll(/\/\/[^\n]*|\/\*[\s\S]*?\*\//g)) {
      const text2 = match[0];
      if (!SUCCESS_CONTRACT.test(text2)) continue;
      const identifier = /\b([A-Z][A-Za-z0-9_]*(?:Latency|Duration)[A-Za-z0-9_]*)\b/.exec(text2)?.[1];
      const operation = /\b([A-Z][A-Za-z0-9]{2,})\s+(?:completion|completed)\b/.exec(text2)?.[1];
      if (identifier === void 0 || operation === void 0) continue;
      contracts.push({
        identifier,
        operation,
        text: normalize2(text2),
        path: file.path,
...
function successGuarded(call, deferredBody, source) {
  for (let node = call.parent; node !== null && node.id !== deferredBody.id; node = node.parent) {
    ...
    const text2 = normalize2(sourceText(condition, source));
    if (/\b(?:success|succeeded|completed)\b/i.test(text2)) return true;
    if (/\b(?:err|error)\s*==\s*nil\b|\bnil\s*==\s*(?:err|error)\b/.test(text2)) return true;
    if (/\bresult\s*!=\s*nil\b|\bnil\s*!=\s*result\b/.test(text2)) return true;
  }
  return false;
}
...
function normalize2(text2) {
  return stripComments(text2).replace(/\s+/g, "");
}
